...
def get_adapter_args(name='vitl', backbone_weight=None,
    ...
    #adapter_model = SAMViTAdapter(**adapter_args)
    return adapter_args
        ...
        if freeze_backbone:
            ...
            if finetune:
                ...
                for idx in finetuned_interaction_indexes:
                    ...
                    for p in self.vit_module.blocks[indexes[-1]-1:indexes[-1]+1].parameters():
    ...
    def forward(self, x):
        if self.freeze_backbone:
            ...
            if self.finetune and self.training:
                ...
                for idx in finetuned_interaction_indexes:
                    indexes = self.interaction_indexes[idx]
                    self.vit_module.blocks[indexes[-1]-1:indexes[-1]+1].train()
                    # self.vit_module.blocks[indexes[0]:indexes[-1] + 1].train()
        else:
            self.vit_module.train()
        ...
        for i, layer in enumerate(self.interactions):
            ...
            # print(x.shape)
            # print('================================================')
            x, c = layer(x, c, self.vit_module.blocks[indexes[0]:indexes[-1] + 1],
...
class SAMAdapter(SAMViTAdapter, Backbone):
    def __init__(self, cfg, input_shape):
        name = cfg.MODEL.SAM_ADAPTER.NAME
        backbone_weight = cfg.MODEL.SAM_ADAPTER.VIT_WEIGHT
        freeze_backbone = cfg.MODEL.SAM_ADAPTER.FREEZE_VIT
        finetune = cfg.MODEL.SAM_ADAPTER.FINETUNE
        finetune_indexes = cfg.MODEL.SAM_ADAPTER.FINETUNE_INDEXES
        with_cp = cfg.MODEL.SAM_ADAPTER.WITH_CP
    ...
    def output_shape(self):
        return {
            name: ShapeSpec(
                channels=self.vit_module.embed_dim, stride=self._out_feature_strides[name]
            )
