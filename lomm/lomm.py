...
from .criterion_lomm import VideoSetCriterion
from .criterion_lomm_E import VideoSetCriterion_E
...
from .lomm_tracker import LOMM_Tracker, LOMM_Tracker_E
...
class MinVIS(nn.Module):
    """
    Modified from "https://github.com/NVlabs/MinVIS".
    Corresponding to Tab. 3-(b).
    """
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "memory_mode": cfg.MODEL.LOMM.MEMORY_MODE,
            "ohm": cfg.MODEL.LOMM.OHM
        }
    ...
    def post_processing(self, outputs):
        ...
        memory = pred_embds[0]
        if self.ohm:
            # We filter out empty masks (Extended from Eq. (5))
            occupancy_memory = torch.logical_and(((pred_masks[0].cuda()>0).float().sum(dim=(1,2))>0), (pred_logits[0].max(dim=-1)[1] != pred_logits[0].shape[-1]-1))        

        # match the instances frame by frame
        for i in range(1, len(pred_logits)):
            curr_obj = pred_embds[i]
            # object matching between memory and current objects
            if self.ohm:
                # occupancy-guided Hungarian matching
                if occupancy_memory.sum() > 0: # There are cases where no object exists in the first frame
                    occupied_obj = memory[occupancy_memory]
                    unoccupied_obj = memory[occupancy_memory==False]
                    
                    # (1) matching with occupied objects
                    occupied_indices = self.match_from_embds(occupied_obj, curr_obj)
                    rest_indices = torch.ones_like(occupancy_memory).bool()
                    rest_indices[occupied_indices]=False
                    
                    # (2) matching with unoccupied objects
                    unoccupied_indices = self.match_from_embds(unoccupied_obj, curr_obj[rest_indices])
                    
                    # get matching result by reordering
                    matching_result = curr_obj.clone()
                    matching_result[occupancy_memory] = curr_obj[occupied_indices]
                    matching_result[occupancy_memory==False] = curr_obj[rest_indices][unoccupied_indices]
                    
                    curr_logits = pred_logits[i].clone()
                    curr_logits[occupancy_memory] = pred_logits[i][occupied_indices]
                    curr_logits[occupancy_memory==False] = pred_logits[i][rest_indices][unoccupied_indices]
                    
                    curr_masks = pred_masks[i].clone()
                    curr_masks[occupancy_memory] = pred_masks[i][occupied_indices]
                    curr_masks[occupancy_memory==False] = pred_masks[i][rest_indices][unoccupied_indices]
                    
                    out_logits.append(curr_logits)
                    out_masks.append(curr_masks)
                else: 
                    indices = self.match_from_embds(memory, pred_embds[i])
                    matching_result = curr_obj[indices]
                    out_logits.append(pred_logits[i][indices, :])
                    out_masks.append(pred_masks[i][indices, :, :])
            
            else:  # Hungarian matching
                indices = self.match_from_embds(memory, curr_obj)
                matching_result = curr_obj[indices]
                out_logits.append(pred_logits[i][indices, :])
                out_masks.append(pred_masks[i][indices, :, :])
            
            # Instance memory update
            if self.memory_mode == 'sim':
                sim = F.cosine_similarity(memory, matching_result, dim =-1).unsqueeze(-1)  # q 1
                memory = (1-sim) * memory + sim * matching_result
            elif self.memory_mode == 'momentum':
                memory = 0.99 * memory + 0.01 * matching_result
            elif self.memory_mode == 'lom':
                fg_prob = 1 - F.softmax(out_logits[-1], dim=-1)[..., -1:]  # q 1
                memory = (1-fg_prob) * memory + fg_prob * matching_result  
            else:  # same with MinVIS
                memory = matching_result
            
            # Occupancy memory update
            if self.ohm:
                curr_occupied = torch.logical_and(((out_masks[-1].cuda()>0).float().sum(dim=(1,2))>0), (out_logits[-1].max(dim=-1)[1] != out_logits[-1].shape[-1]-1))
                occupancy_memory = torch.logical_or(occupancy_memory, curr_occupied)
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        for i in range(iters):
            ...
            del out['pred_embds_without_norm'], out['mask_features']
    ...
    def inference_video(self, pred_cls, pred_masks, img_size, output_height, output_width, first_resize_size):
        if len(pred_cls) > 0:
            ...
            labels = torch.arange(
                ...
                device=self.device
            ).unsqueeze(0).repeat(pred_cls.shape[0], 1).flatten(0, 1)
...
@META_ARCH_REGISTRY.register()
class LOMM_online_E(MinVIS):
    """
    Only train T_E.
    Corresponding to Tab. 3-(c).
    """
        ...
        *,
        ...
        Args:
            ...
            pixel_mean, pixel_std: list or tuple with #channels element, representing
                the per-channel mean and std to be used to normalize the input image
            # video
            tracker_E: an tracker module that tracks existing objects
    ...
    def from_config(cls, cfg):
        ...
        weight_dict = {
            ...
            "loss_ce_E": class_weight,
            "loss_mask_E": mask_weight,
            "loss_dice_E": dice_weight,
        }
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            with torch.no_grad():
                ...
                # fg prob
                cls_probs = image_outputs['pred_logits'].clone().detach()  # b t q c
                fg_prob = 1 - F.softmax(cls_probs, dim=-1)[..., -1:]  # b t q 1
                fg_prob = fg_prob.permute(1,2,0,3)  # t q b 1
                
                # We filter out empty masks. Implemetned only for batch 1
                is_fg = torch.logical_and(((image_outputs['pred_masks'].transpose(1, 2) > 0).float().sum(dim=(-2,-1))>0),
                        (image_outputs['pred_logits'].max(dim=-1)[1] != self.sem_seg_head.num_classes))[0]  # t q

                frame_embds = image_outputs['pred_embds'].clone().detach()  # (b, c, t, q)
                frame_embds_no_norm = image_outputs['pred_embds_without_norm'].clone().detach()
                mask_features = image_outputs['mask_features'].clone().detach().unsqueeze(0)
                del image_outputs['mask_features']
                torch.cuda.empty_cache()
            outputs, indices = self.tracker_E(frame_embds, mask_features, fg_prob, is_fg, return_indices=True, resume=self.keep,
                                            frame_embeds_no_norm=frame_embds_no_norm, early_matching=True)
            image_outputs = self.reset_image_output_order(image_outputs, indices)

        if self.training:
            targets = self.prepare_targets(batched_inputs, images)
            # use the segmenter prediction results to guide the matching process
    ...
    def frame_decoder_loss_reshape(self, outputs, targets, image_outputs=None):
        outputs['pred_masks_E'] = einops.rearrange(outputs['pred_masks_E'], 'b q t h w -> (b t) q () h w')
        outputs['pred_logits_E'] = einops.rearrange(outputs['pred_logits_E'], 'b t q c -> (b t) q c')
        ...
        if 'aux_outputs_E' in outputs:
            for i in range(len(outputs['aux_outputs_E'])):
                outputs['aux_outputs_E'][i]['pred_masks'] = einops.rearrange(
                    outputs['aux_outputs_E'][i]['pred_masks'], 'b q t h w -> (b t) q () h w'
                )
                outputs['aux_outputs_E'][i]['pred_logits'] = einops.rearrange(
                    outputs['aux_outputs_E'][i]['pred_logits'], 'b t q c -> (b t) q c'
                )
        gt_instances = []
        for targets_per_video in targets:
            num_labeled_frames = targets_per_video['ids'].shape[1]
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        for i in range(iters):
            ...
            # fg prob
            cls_probs = out['pred_logits'].clone().detach()  # b t q c
            fg_prob = 1 - F.softmax(cls_probs, dim=-1)[..., -1:]  # b t q 1
            fg_prob = fg_prob.permute(1,2,0,3)  # t q b 1
            
            is_fg = torch.logical_and(((out['pred_masks'].transpose(1, 2) > 0).float().sum(dim=(-2,-1))>0),
                    (out['pred_logits'].max(dim=-1)[1] != self.sem_seg_head.num_classes))[0]  # t q

            if i != 0 or self.keep:
                track_out, indices = self.tracker_E(frame_embds, mask_features, fg_prob, is_fg, return_indices=True, resume=True, 
                                         frame_embeds_no_norm=frame_embds_no_norm)
            else:
                track_out, indices = self.tracker_E(frame_embds, mask_features, fg_prob, is_fg, return_indices=True,
                                         frame_embeds_no_norm=frame_embds_no_norm)
            # Reordering by OHM
            out = self.reset_image_output_order(out, indices) 
            
            # remove unnecessary variables to save GPU memory
            del mask_features, out['mask_features'], track_out['pred_embds']
            del track_out['pred_logits_E'], track_out['pred_masks_E']
            for j in range(len(track_out['aux_outputs_E'])):
                del track_out['aux_outputs_E'][j]['pred_masks'], track_out['aux_outputs_E'][j]['pred_logits']
            track_out['pred_logits'] = out['pred_logits'].to(torch.float32).detach().cpu()
            track_out['pred_masks'] = out['pred_masks'].to(torch.float32).detach().cpu()
            track_out['pred_embds'] = out['pred_embds'].to(torch.float32).detach().cpu()
...
@META_ARCH_REGISTRY.register()
class LOMM_online(MinVIS):
    """
    LOMM full model: train T_E + T_A
    """
        ...
        *,
        ...
        Args:
            ...
            pixel_mean, pixel_std: list or tuple with #channels element, representing
                the per-channel mean and std to be used to normalize the input image
            # video
            tracker: a tracker module, which consists of T_E and T_A
            num_frames: number of frames sampled during training
            window_inference: if the GPU memory is insufficient to predict the entire video at
                once, inference needs to be performed clip by clip
            num_class: the categories number of the dataset
    ...
    def from_config(cls, cfg):
        ...
        dice_weight = cfg.MODEL.MASK_FORMER.DICE_WEIGHT
        mask_weight = cfg.MODEL.MASK_FORMER.MASK_WEIGHT
        sim_weight = cfg.MODEL.LOMM.SIM_WEIGHT
        ...
        return {
            ...
            "task": cfg.MODEL.MASK_FORMER.TEST.TASK,
            # lomm
            "early_matching": cfg.MODEL.LOMM.EARLY_MATCHING
        }
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            outputs, indices = self.tracker(frame_embds, mask_features, fg_prob, return_indices=True, resume=self.keep,
                                            frame_embeds_no_norm=frame_embds_no_norm, early_matching=((self.iter < self.max_iter_num // 2) and self.early_matching))
            image_outputs = self.reset_image_output_order(image_outputs, indices)
        ...
        if self.training:
            ...
            if (self.iter < self.max_iter_num // 2) and self.early_matching:
                losses = self.criterion(outputs, targets, matcher_outputs=image_outputs)
            else:
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        for i in range(iters):
            ...
            if i != 0 or self.keep:
                track_out = self.tracker(frame_embds, mask_features, fg_prob, resume=True, 
                                         frame_embeds_no_norm=frame_embds_no_norm)
            else:
                track_out = self.tracker(frame_embds, mask_features, fg_prob,
                                         frame_embeds_no_norm=frame_embds_no_norm)
                
            # remove unnecessary variables to save GPU memory
            del mask_features
            del track_out['sim_scores'], track_out['pred_logits_E'], track_out['pred_masks_E']
...
@META_ARCH_REGISTRY.register()
class LOMM_offline(LOMM_online):
    """
    modified from "https://github.com/zhang-tao-whu/DVIS/blob/main/dvis/meta_architecture.py"
    """
    @configurable
    def __init__(
        self,
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            with torch.no_grad():
                ...
                image_outputs = self.segmentor_windows_inference(images.tensor, window_size=15)
                ...
                image_outputs = self.tracker(
                    frame_embds, mask_features, fg_prob,
                    resume=self.keep,
                    frame_embeds_no_norm=frame_embds_no_norm
                )
                ...
                del image_outputs['pred_logits_E'], image_outputs['pred_masks_E']
                ...
                for j in range(len(image_outputs['aux_outputs_E'])):
                    del image_outputs['aux_outputs_E'][j]['pred_masks'], image_outputs['aux_outputs_E'][j]['pred_logits']
                torch.cuda.empty_cache()
            # do temporal refine
